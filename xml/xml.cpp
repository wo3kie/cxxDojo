...
void print(boost::property_tree::ptree const& pt, int indent = 0) {
  for(auto const& element : pt) {
    if(element.first == "name") {
      ...
      std::cout << std::string(indent, ' ') << elementName << " " << elementValue << std::endl;

      for(auto const& attr : attrs) {
        auto const& attributeName = attr.first;
        auto const& attributeValue = attr.second.get_value<std::string>();

        std::cout << std::string(indent + 4, ' ') << attributeName << ": " << attributeValue << std::endl;
      }
    } else if(element.first == "doc") {
      std::cout << std::string(indent, ' ') << element.first << std::endl;
    }
