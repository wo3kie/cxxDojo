...
Return lfold(Container c, std::function<Return(typename Container::value_type, typename Container::value_type)> f) {
  return _lfold(f, *c.begin(), std::next(c.begin()), c.end());
}

template<typename R, typename T, typename I>
R _rfold(std::function<R(T, T)> f, R init, I begin, I end) {
...
template<typename Return, typename Container>
Return rfold(Container c, std::function<Return(typename Container::value_type, typename Container::value_type)> f) {
...
typename Container::value_type sum(Container container) {
  ...
  std::function<value_type(value_type, value_type)> f = [](value_type lhs, value_type rhs) -> value_type {
    return lhs + rhs;
  };

  return lfold(container, f);
}
...
Container& filter(Container& container, std::function<bool(typename Container::value_type)> predicate) {
  typename Container::iterator toBeRemoved = std::partition(container.begin(), container.end(), predicate);
