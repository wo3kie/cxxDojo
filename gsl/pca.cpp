 ...
 *
 * Compilation:
 *      g++ --std=c++20 pca.cpp -o pca -lgsl -lgslcblas -lm
 *
 * Usage:
 *      $ ./pca
 */
...
gsl_matrix* pca(gsl_matrix const* const matrix /* (points, dim) */, unsigned newDimension) {
  ...
  for(unsigned i = 0; i < dim; ++i) {
    gsl_vector_set(mean, i, gsl_stats_mean(matrix->data + i, dim, points));
  }

  /* Subtract mean
     */

  gsl_matrix* const mean_subtracted_matrix = gsl_matrix_alloc(points, dim);

  gsl_matrix_memcpy(mean_subtracted_matrix, matrix);

  for(unsigned i = 0; i < points; ++i) {
    gsl_vector_view mean_subtracted_view = gsl_matrix_row(mean_subtracted_matrix, i);
  ...
  gsl_eigen_symmv_workspace* const workspace = gsl_eigen_symmv_alloc(dim);
  ...
  gsl_matrix_view const reduced_eigenvectors = gsl_matrix_submatrix(eigenvectors, 0, 0, dim, newDimension);
  ...
  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, matrix, &reduced_eigenvectors.matrix, 0.0, result);
