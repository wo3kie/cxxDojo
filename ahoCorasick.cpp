 ...
 *
 * Compilation:
 *      g++ --std=c++11 ahoCorasick.cpp -o ahoCorasick
 *
 * Usage:
 *      $ ./ahoCorasick
 */
...
std::ostream & operator<<( std::ostream & out, Node const & node ){
    if( node.parent_ ){
        out
            << "(" << (&node) << ", " << node.letter_ << ", " << node.end_
            << ", (" << node.failure_ << ", " << node.failure_->letter_ << "))";
    }
    else{
        out << "(" << (&node) << ", , , (,)))";
    }
...
struct AhoCorasick {
    ...
    void search(
        ...
        std::function< void ( size_t const, std::string const & ) > const & f
    ) const {
