 ...
 *
 * Compilation:
 *      g++ --std=c++20 sign.cpp -o sign -lssl
 *
 * Usage:
 *      $ ./sign
 */
...
RSA* getPrivateKey(std::string const& privateKeyPemString) {
  BIO* bioBuffer = BIO_new_mem_buf((void*)privateKeyPemString.c_str(), (int)privateKeyPemString.size());

  RSA* result = PEM_read_bio_RSAPrivateKey(
      bioBuffer, nullptr, nullptr, (void*)"1234" /* password */
  );
...
RSA* getPublicKey1(std::string const& publicKeyPemString) {
  BIO* const bioBuffer = BIO_new_mem_buf((void*)publicKeyPemString.c_str(), (int)publicKeyPemString.size());

  //RSA* const rsaPublic = PEM_read_bio_RSAPublicKey(
  RSA* const result = PEM_read_bio_RSA_PUBKEY(bioBuffer, nullptr, nullptr, nullptr);
...
RSA* getPublicKey3(RSA* rsaPrivate) {
  ...
  int const size = i2d_RSAPublicKey(rsaPrivate, (unsigned char**)(&buffer));

  RSA* const rsaPublic = d2i_RSAPublicKey(nullptr, (unsigned char const**)(&buffer), size);
...
int main() {
  ...
  const std::string privateKeyPemString("-----BEGIN RSA PRIVATE KEY-----\n"
                                        "Proc-Type: 4,ENCRYPTED\n"
                                        "DEK-Info: DES-EDE3-CBC,F263834F4AB2BA3F\n"
                                        "\n"
                                        "1NvFchbMolZoh7jIthMyYTvpr3hwVNa1KmhGohzxY0M0AhTxBOHPEXL2ZIbKsf4v\n"
  ...
  const std::string publicKeyPemString("-----BEGIN PUBLIC KEY-----\n"
                                       "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCWCbDaq8HYPdYg16wBZj9+CyZq\n"
                                       "7cg4hHMJmKyQtJr50vTLwecaO9e16ynD1ZuZyeOm3sZqlRBlRChUTrAJ7H2haUzG\n"
                                       "aHn8kL9gaIafo/Mhd0BoCOSDkcqmbLWDtj7EmZTbYC7Isg/ZBD9g4YcDaZflU68m\n"
                                       "zY5m3v7qcKmusf/q3wIDAQAB\n"
  ...
  const ustring_t messageString(_messageString.begin(), _messageString.end());
  ...
  SHA1(messageString.c_str(), messageString.size(), sha1);
...

  {
    ...
    int const signResult = RSA_sign(NID_sha1, sha1, SHA_DIGEST_LENGTH, signatureBuffer, &signatureSize, rsaPrivate);
...

  {
    ...
    int const verifyResult = RSA_verify(NID_sha1, sha1, SHA_DIGEST_LENGTH, signatureBuffer, signatureSize, rsaPublic);
