...
public:
        ...
        , responseCallback_(responseCallback)
    {
        ...
        socket_.set_verify_callback(
            boost::bind(
                &HttpsClient::verify_certificate,
                this,
                _1,
                _2
            )
...
int main(int argc, char* argv[]){
    try{
        ...
        std::string const basicAuthenticationToken = base64encode(
            bingKeyId + ":" + bingKeyId
        );

        std::string const response = bingWebSearchQuery(
            query,
            basicAuthenticationToken
        );
