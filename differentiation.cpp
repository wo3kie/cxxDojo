...
int main(){
    auto f = [](double x){ return 2*x*x*x*x - 4*x*x*x + 6*x*x - 8*x + 10;};
    auto df_dx = [](double x){ return 2*4*x*x*x - 4*3*x*x + 6 * 2 * x - 8;};
    auto d2f_dx2 = [](double x){ return 2*4*3*x*x - 4*3*2*x + 6 * 2;};
    auto d3f_dx3 = [](double x){ return 2*4*3*2*x - 4*3*2;};
