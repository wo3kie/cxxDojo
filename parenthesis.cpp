...
parenthesisImpl(int open, int close, int maxOpen, int maxClose, std::vector<std::string> solutions1) {
  ...
  if(open + 1 <= maxOpen) {
    ...
    solutions1.insert(solutions1.end(), solutions2.begin(), solutions2.end());
  }
...
std::vector<std::string> parenthesis(int number) {
  ...
  return parenthesisImpl(0, 0, number, number, strings);
}
