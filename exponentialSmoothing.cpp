...
double meanSquaredError(
    Iterator expectedBegin,
    Iterator const expectedEnd,
    Iterator2 actualBegin
)
{
...
std::vector<double> findBestExponentialSmoothing(
    ...
    for(double alpha2 = from; alpha2 < to; alpha2 += (to - from) / 10 ){
        std::vector<double> smoothed2 = exponentialSmoothing(
            values,
            alpha2
        );

        double const error2 = meanSquaredError(
            values.begin() + 1,
            values.end(),
            smoothed2.begin()
        );
...
int main(){
    ...
    std::cout
        << findBestExponentialSmoothing(values, alpha)
        << "\n";
